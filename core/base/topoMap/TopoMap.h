...
namespace ttk {
  ...
  int ttk::TopoMap::execute(T *outputCoords,
    ...
    for(const auto &elt : edgeHeapVect) {
      ...
      if(!statusHull) {
        this->printErr(
          "Error: could not retrieve all points from the convex hull.");
        return 1;
      }
...
bool computeConvexHull(T *allCoords,
  ...
  for(size_t i = 0; i < nbPoint; i++) {
    ...
    compCoords[2 * i] = truncateDouble(allCoords[2 * vertId]);
    compCoords[2 * i + 1] = truncateDouble(
      allCoords[2 * vertId + 1]); // To avoid bugs in boost::geometry to compute
                                  // the convexhull on at least one example,
                                  // when type T is double.
  }
  ...
  for(size_t iPt = idFirstDistinct + 1; iPt < nbPoint; iPt++) {
    T curVect[2] = {compCoords[2 * iPt] - compCoords[0],
  ...
  for(size_t i = 0; i < nbPoint; i++) {
    boost::geometry::append(
      multiPoints, Point(compCoords[2 * i], compCoords[2 * i + 1]));
  }
  boost::geometry::convex_hull(multiPoints, hull);
  for(const auto &boostPt : hull.outer()) {
    T coordsCur[2]
      = {static_cast<T>(boostPt.get<0>()), static_cast<T>(boostPt.get<1>())};
  ...
  if(idsInHull.size() != hull.outer().size())
    return false;
