...
vtkStandardNewMacro(ttkTopoMapHarishd);
...
static void extractInputMatrix(std::vector<std::vector<float>> &distMatrix,
  ...
  if (nThreads == 1000000)
    std::cerr << "J'utilise nThreads la variable no warning";
  if(useRegexp) {
...
ttkTopoMapHarishd::ttkTopoMapHarishd() {
  this->SetNumberOfInputPorts(1);
  this->SetNumberOfOutputPorts(1);
}


int ttkTopoMapHarishd::FillInputPortInformation(int port, vtkInformation *info) {
  if(port == 0) {
...
int ttkTopoMapHarishd::FillOutputPortInformation(int port, vtkInformation *info) {
  if(port == 0) {
...
int ttkTopoMapHarishd::RequestData(vtkInformation *ttkNotUsed(request),
  ...
  vtkDoubleArray *colX = vtkDoubleArray::SafeDownCast(input->GetColumnByName("X")),
                 ...
                 *colZ = vtkDoubleArray::SafeDownCast(input->GetColumnByName("Z"));
  //

  std::vector<double> inputCoords(nbPoint*3);

  for (size_t i = 0; i < nbPoint; i++)
  {
    inputCoords[3*i] = colX->GetValue(i);
    inputCoords[3*i+1] = colY->GetValue(i);
    inputCoords[3*i+2] = colZ->GetValue(i);
  }
  ...
  this->execute<float>(ttkUtils::GetPointer<float>(outputPoints->GetData()), inputCoords);
